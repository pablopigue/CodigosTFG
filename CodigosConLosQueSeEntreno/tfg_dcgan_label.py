...
if DATASET_NAME in ["MNIST", "FashionMNIST"]:
    ...
    raise ValueError(f"Dataset '{DATASET_NAME}' no reconocido. Opciones válidas: MNIST, FashionMNIST, SVHN, Gatos.")

EXPERIMENT_DIR = f"/mnt/homeGPU/pablomarpa/CodigosTFG/tfg_dcgan_ls_{DATASET_NAME.lower()}"
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(EPOCHS):
        ...
        if run == 0 and ((epoch + 1) % SAVE_IMG_FREQ == 0 or epoch == 0):
            with torch.no_grad():
                ...
                plt.axis('off')
                plt.title(f"Imágenes Generadas DCGAN+LS ({DATASET_NAME}) - Época {epoch+1}")
    ...
    if run == 0:
        ...
        save_plot(
            ...
            colors=['blue', 'orange'],
            title=f'Curvas de Aprendizaje DCGAN+LS - {DATASET_NAME}',
...
save_plot(
    ...
    labels=['Pérdida del Generador', 'Pérdida del Discriminador'],
    colors=['blue', 'orange'],
    title=f'Curvas de Aprendizaje Promediadas DCGAN+LS - {DATASET_NAME}',
    xlabel='Épocas', ylabel='Pérdida',
...
save_plot(
    ...
    labels=['Puntuación FID'], colors=['green'],
    title=f'Evolución de la Calidad FID Promediada DCGAN+LS - {DATASET_NAME}',
    xlabel='Épocas', ylabel='FID',
...
save_plot(
    ...
    labels=['Puntuación Inception (media)'], colors=['purple'],
    title=f'Evolución del Inception Score Promediado DCGAN+LS - {DATASET_NAME}',
    xlabel='Épocas', ylabel='IS',
