...
EXPERIMENT_DIR = f"/mnt/homeGPU/pablomarpa/CodigosTFG/tfg_dcgan_{DATASET_NAME.lower()}"
...
print(f"Iniciando entrenamiento DCGAN en: {DEVICE} con dataset {DATASET_NAME}. Ejecuciones totales: {NUM_RUNS}", flush=True)
...
# Arquitectura DCGAN adaptada a 64x64.
# Generador: 1x1 -> 4x4 -> 8x8 -> 16x16 -> 32x32 -> 64x64
# Discriminador: 64x64 -> 32x32 -> 16x16 -> 8x8 -> 4x4 -> 1x1
# ==========================================
def weights_init(m):
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, data in enumerate(loader):
            ...
            loss_disc_real = criterion(output_real, torch.ones(batch_size_curr, device=DEVICE))
        ...
        if run == 0 and ((epoch + 1) % SAVE_IMG_FREQ == 0 or epoch == 0):
            with torch.no_grad():
                ...
                plt.axis('off')
                plt.title(f"Imágenes Generadas DCGAN ({DATASET_NAME}) - Época {epoch+1}")
    ...
    if run == 0:
        ...
        save_plot(
            ...
            colors=['blue', 'orange'],
            title=f'Curvas de Aprendizaje DCGAN - {DATASET_NAME}',
...
save_plot(
    ...
    labels=['Pérdida del Generador', 'Pérdida del Discriminador'],
    colors=['blue', 'orange'],
    title=f'Curvas de Aprendizaje Promediadas DCGAN - {DATASET_NAME}',
    xlabel='Épocas', ylabel='Pérdida',
...
save_plot(
    ...
    labels=['Puntuación FID'], colors=['green'],
    title=f'Evolución de la Calidad FID Promediada DCGAN - {DATASET_NAME}',
    xlabel='Épocas', ylabel='FID',
...
save_plot(
    ...
    labels=['Puntuación Inception (media)'], colors=['purple'],
    title=f'Evolución del Inception Score Promediado DCGAN - {DATASET_NAME}',
    xlabel='Épocas', ylabel='IS',
    filepath=f"{EXPERIMENT_DIR}/plots/is_metric_mean.png",
    stds=[df_std_metrics['is_mean']], markers=['o']
)

print(f"Entrenamiento DCGAN de {NUM_RUNS} ejecuciones finalizado. Todo guardado en: {EXPERIMENT_DIR}", flush=True)
